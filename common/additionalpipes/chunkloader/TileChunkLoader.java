...
public class TileChunkLoader extends TileEntity
{
	...
	{
		final List<ChunkCoordIntPair> loadArea = new LinkedList<ChunkCoordIntPair>();

		for (int x = -loadDistance; x < (loadDistance + 1); x++)
		{
			for (int z = -loadDistance; z < (loadDistance + 1); z++)
			{
				final ChunkCoordIntPair chunkCoords = new ChunkCoordIntPair((xCoord >> 4) + x, (zCoord >> 4) + z);
	...
	public void validate()
	{
		...
		{
			final Ticket ticket = ForgeChunkManager.requestTicket(AdditionalPipes.instance, worldObj, Type.NORMAL);
			if (ticket != null)
			{
	...
	public void forceChunkLoading(Ticket ticket)
	{
		stopChunkLoading();
		chunkTicket = ticket;
		for (final ChunkCoordIntPair coord : getLoadArea())
		{
	...
	{
		for (final Object obj : chunkTicket.getChunkList())
		{
			final ChunkCoordIntPair coord = (ChunkCoordIntPair) obj;
