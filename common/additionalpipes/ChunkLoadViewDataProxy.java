...
import additionalpipes.network.NetworkHandler;
import additionalpipes.network.PacketAdditionalPipes;
...
public class ChunkLoadViewDataProxy implements IScheduledTickHandler, Comparator<ChunkCoordIntPair>
{
	...
	public void activateLasers()
	{
		...
		for (final ChunkCoordIntPair coords : persistentChunks)
		{
			final int xCoord = coords.chunkXPos * 16;
			final int zCoord = coords.chunkZPos * 16;
	...
	{
		final PacketAdditionalPipes packet = new PacketAdditionalPipes(NetworkHandler.CHUNKLOAD_REQUEST, false);
	...
	public void sendPersistentChunksToPlayer(EntityPlayerMP player)
	{
		...
		final SetMultimap<ChunkCoordIntPair, Ticket> persistentChunks = ForgeChunkManager.getPersistentChunksFor(player.worldObj);
		final List<ChunkCoordIntPair> chunksInRange = new LinkedList<ChunkCoordIntPair>();
		final int playerX = (((int) player.posX) >> 4) - (sightRange / 2), playerZ = (((int) player.posZ) >> 4) - (sightRange / 2);

		for (int i = -sightRange; i <= sightRange; i++)
		{
			for (int j = -sightRange; j <= sightRange; j++)
			{
				final ChunkCoordIntPair coords = new ChunkCoordIntPair(playerX + i, playerZ + j);
		...
		final PacketAdditionalPipes packet = new PacketAdditionalPipes(NetworkHandler.CHUNKLOAD_DATA, false);
		packet.writeInt(chunksInRange.size());
		for (final ChunkCoordIntPair coords : chunksInRange)
		{
	...
	{
		final int dx = first.chunkXPos - other.chunkXPos;
