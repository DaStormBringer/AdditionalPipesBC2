...
import additionalpipes.network.PacketNBTTagData;
...
public class ContainerTeleportPipe extends BuildCraftContainer
{
	...
	public ContainerTeleportPipe(EntityPlayer player, PipeTeleport pipe)
	{
		...
		final List<PipeTeleport> connectedPipes = TeleportManager.instance.getConnectedPipes(pipe, false);
		final int[] locations = new int[connectedPipes.size() * 3];
		for (int i = 0; (i < connectedPipes.size()) && (i < 9); i++)
		{
			final Pipe connectedPipe = connectedPipes.get(i);
		...
		final PacketNBTTagData packet = new PacketNBTTagData(AdditionalPipes.CHANNELNBT, NetworkHandler.TELE_PIPE_DATA, false, tag);
