...
public class GuiTeleportPipe extends GuiBuildCraft
{
	...
	{
		super.initGui();
		final int x = (width - xSize) / 2, y = ((height - ySize) / 2) + 16;
		final int bw = xSize - 24;
	...
	protected void drawGuiContainerForegroundLayer(int p1, int p2)
	{
		...
		fontRenderer.drawString(new StringBuilder("(").append(pipe.container.xCoord).append(", ").append(pipe.container.yCoord).append(", ").append(pipe.container.zCoord).append(")").toString(), 128, 12, 0x404040);
		switch (pipe.state)
		{
			case 3:
	...
	protected void actionPerformed(GuiButton guibutton)
	{
		...
		final PacketAdditionalPipes packet = new PacketAdditionalPipes(NetworkHandler.TELE_PIPE_DATA_SET, false);
