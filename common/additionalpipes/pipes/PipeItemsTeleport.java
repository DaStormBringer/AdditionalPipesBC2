...
public class PipeItemsTeleport extends PipeTeleport implements IPipeTransportItemsHook
{
	...
	{
		if (!AdditionalPipes.proxy.isServer(getWorld())) return;
		final List<PipeTeleport> connectedTeleportPipes = TeleportManager.instance.getConnectedPipes(this, false);
		// no teleport pipes connected, use default
		if ((connectedTeleportPipes.size() <= 0) || ((state & 0x1) == 0)) return;

		// output to random pipe
		final LinkedList<ForgeDirection> outputOrientations = new LinkedList<ForgeDirection>();
		final PipeTeleport otherPipe = connectedTeleportPipes.get(rand.nextInt(connectedTeleportPipes.size()));

		// find possible output orientations
		for (final ForgeDirection o : ForgeDirection.VALID_DIRECTIONS)
		{
		...
		if (outputOrientations.size() <= 0) return;

		final ForgeDirection newOrientation = outputOrientations.get(rand.nextInt(outputOrientations.size()));
		final TileGenericPipe destination = (TileGenericPipe) otherPipe.container.getTile(newOrientation);
