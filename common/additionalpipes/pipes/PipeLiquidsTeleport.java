...
public class PipeLiquidsTeleport extends PipeTeleport implements IPipeTransportFluidsHook
{
	...
	{
		final List<PipeTeleport> pipeList = TeleportManager.instance.getConnectedPipes(this, false);

		if ((pipeList.size() == 0) || ((state & 0x1) == 0)) return 0;

		final int i = getWorld().rand.nextInt(pipeList.size());
		final List<IFluidHandler> possibleMovements = getPossibleLiquidMovements(pipeList.get(i));

		if (possibleMovements.size() <= 0) return 0;

		int used = 0;
		while ((possibleMovements.size() > 0) && (used <= 0))
		{
			final int a = rand.nextInt(possibleMovements.size());
	...
	{
		final List<IFluidHandler> result = new LinkedList<IFluidHandler>();

		for (final ForgeDirection o : ForgeDirection.VALID_DIRECTIONS)
		{
			if (pipe.outputOpen(o))
			{
				final IFluidHandler te = (IFluidHandler) pipe.container.getTile(o);
				result.add(te);
			}
		}
