...
public class TeleportManager
{
	...
	{
		if (!AdditionalPipes.proxy.isServer(pipe.getWorld())) return;
		teleportPipes.add(pipe);
		AdditionalPipes.instance.logger.info(String.format("[TeleportManager] Pipe added: %s @ (%d, %d, %d), %d pipes in network", pipe.getClass().getSimpleName(), pipe.container.xCoord, pipe.container.yCoord, pipe.container.zCoord, teleportPipes.size()));
	}
	...
	public void remove(PipeTeleport pipe)
	{
		...
		AdditionalPipes.instance.logger.info(String.format("[TeleportManager] Pipe removed: %s @ (%d, %d, %d), %d pipes in network", pipe.getClass().getSimpleName(), pipe.container.xCoord, pipe.container.yCoord, pipe.container.zCoord, teleportPipes.size()));
	}
	...
	{
		final List<PipeTeleport> connected = new LinkedList<PipeTeleport>();

		for (final PipeTeleport other : teleportPipes)
		{
			...
			if (((pipe.container.xCoord != other.container.xCoord) || (pipe.container.yCoord != other.container.yCoord) || (pipe.container.zCoord != other.container.zCoord)) && (other.getFrequency() == pipe.getFrequency()) && (((other.state & 0x2) > 0) || forceReceive) && (pipe.isPublic ? other.isPublic : other.owner.equalsIgnoreCase(pipe.owner)))
			{
				connected.add(other);
			}
		}
	...
	{
		final List<PipeTeleport> pipes = new LinkedList<PipeTeleport>();
	...
	{
		final String name = frequencyNames.get(frequency);
