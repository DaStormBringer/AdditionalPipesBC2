...
public class PipeTransportAdvancedWood extends PipeTransportItems implements IInventory
{
	...
	{
		final int meta = container.getBlockMetadata();
		int newMeta = 6;

		for (int i = meta + 1; i <= (meta + 6); ++i)
		{
			final ForgeDirection o = ForgeDirection.VALID_DIRECTIONS[i % 6];
			final TileEntity tile = container.getTile(o);
			if (isInput(tile)) if (PipeManager.canExtractItems(container.getPipe(), tile.worldObj, tile.xCoord, tile.yCoord, tile.zCoord))
			{
				newMeta = o.ordinal();
				break;
			}
	...
	private void switchSourceIfNeeded()
	{
		...
		{
			final TileEntity tile = container.getTile(ForgeDirection.VALID_DIRECTIONS[meta]);
			if (!isInput(tile))
			{
				switchSource();
			}
	...
	public void readFromNBT(NBTTagCompound nbttagcompound)
	{
		...
		{
			final NBTTagCompound nbttagcompound2 = (NBTTagCompound) nbttaglist.tagAt(j);
			final int index = nbttagcompound2.getInteger("index");
