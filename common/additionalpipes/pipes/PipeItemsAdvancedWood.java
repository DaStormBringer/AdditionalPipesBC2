...
public class PipeItemsAdvancedWood extends APPipe implements IPowerReceptor
{
	...
	public void doWork(PowerHandler workProvider)
	{
		...
		final int meta = w.getBlockMetadata(container.xCoord, container.yCoord, container.zCoord);

		if (meta > 5) return;

		final Position pos = new Position(container.xCoord, container.yCoord, container.zCoord, ForgeDirection.VALID_DIRECTIONS[meta]);
		pos.moveForwards(1);
		final int blockId = w.getBlockId((int) pos.x, (int) pos.y, (int) pos.z);
		final TileEntity tile = w.getBlockTileEntity((int) pos.x, (int) pos.y, (int) pos.z);
		...
		if (tile instanceof IInventory)
		{
			...
			final ItemStack extracted = checkExtract(inventory, true, pos.orientation.getOpposite());
			...
			final Position entityPos = new Position(pos.x + 0.5, pos.y + CoreConstants.PIPE_MIN_POS, pos.z + 0.5, pos.orientation.getOpposite());
			entityPos.moveForwards(0.5);
			final TravelingItem entity = new TravelingItem(entityPos.x, entityPos.y, entityPos.z, extracted);
	...
	{
		final IInventory inv = Utils.getInventory(inventory);
		final int first = 0;
		final int last = inv.getSizeInventory() - 1;
		if (inventory instanceof ISidedInventory)
		{
			final ISidedInventory sidedInv = (ISidedInventory) inventory;
			final int[] accessibleSlots = sidedInv.getAccessibleSlotsFromSide(from.ordinal());
			final ItemStack result = checkExtractGeneric(inv, doRemove, from, accessibleSlots);
			return result;
		}
		final ItemStack result = checkExtractGeneric(inv, doRemove, from, first, last);
		return result;
	}
	...
	public ItemStack checkExtractGeneric(IInventory inventory, boolean doRemove, ForgeDirection from, int start, int stop)
	{
		...
		{
			final ItemStack slot = inventory.getStackInSlot(k);
	...
	public ItemStack checkExtractGeneric(IInventory inventory, boolean doRemove, ForgeDirection from, int[] slots)
	{
		...
		{
			final ItemStack slot = inventory.getStackInSlot(i);
	...
	public boolean canExtract(ItemStack item)
	{
		...
		{
			final ItemStack stack = transport.getStackInSlot(i);
			if ((stack != null) && (stack.itemID == item.itemID))
			{
				if ((Item.itemsList[item.itemID].isDamageable()))
					return !transport.exclude;
				else if (stack.getItemDamage() == item.getItemDamage()) return !transport.exclude;
			}
		}
		return transport.exclude;
	}
	...
	public int getIconIndex(ForgeDirection direction)
	{
		...
		{
			final int metadata = container.getBlockMetadata();
	...
	{
		final Item equipped = entityplayer.getCurrentEquippedItem() != null ? entityplayer.getCurrentEquippedItem().getItem() : null;
		...
		if (AdditionalPipes.isPipe(equipped)) return false;
