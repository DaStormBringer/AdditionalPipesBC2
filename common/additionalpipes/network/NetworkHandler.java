...
public class NetworkHandler implements IPacketHandler
{
	...
	public void onPacketData(INetworkManager manager, Packet250CustomPayload packet, Player player)
	{
		if (AdditionalPipes.CHANNEL.equals(packet.channel))
		{
			...
			try
			{
				...
				{
					case ADV_WOOD_DATA:
						final TileEntity te = getTileEntity(player, data);
						if (te instanceof TileGenericPipe)
						{
							final PipeItemsAdvancedWood pipe = (PipeItemsAdvancedWood) ((TileGenericPipe) te).pipe;
		...
		{
			final NBTTagCompound tag = PacketNBTTagData.getNBTFrom(packet);
			switch (tag.getInteger("id"))
			{
				case TELE_PIPE_DATA:
					final TileEntity te = ((EntityPlayer) player).worldObj.getBlockTileEntity(tag.getInteger("xCoord"), tag.getInteger("yCoord"), tag.getInteger("zCoord"));
					if (te instanceof TileGenericPipe)
					{
						final PipeTeleport pipe = (PipeTeleport) ((TileGenericPipe) te).pipe;
	...
	private void handleDistPipeData(Player player, DataInputStream data)
	{
		try
		{
			...
			if (te instanceof TileGenericPipe)
			{
				final int index = data.read();
				final int newData = data.readInt();
				final PipeItemsDistributor pipe = (PipeItemsDistributor) ((TileGenericPipe) te).pipe;
				...
				if ((newData >= 0) && (index >= 0) && (index < pipe.distData.length))
				{
					...
					if (!found)
					{
						for (final int element : pipe.distData)
						{
							if (element > 0)
							{
								found = true;
	...
	private void handleChunkLoadData(DataInputStream data)
	{
		try
		{
			...
			final ChunkCoordIntPair[] chunks = new ChunkCoordIntPair[size];
